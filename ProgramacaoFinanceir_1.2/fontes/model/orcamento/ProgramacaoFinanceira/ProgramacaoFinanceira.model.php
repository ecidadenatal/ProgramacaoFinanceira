...
class ProgramacaoFinanceira {
  ...
  private function processarFechamento() {
    ...
    foreach ($this->getFichas() as $oFicha) {
      ...
      if (round($nValorTotal, 2) != round($nValorOrcado, 2)) {
  ...
  /**
   * Retorna a programação financeira para a instituição e ano
   * @param int         $iAno
   * @param Instituicao $oInstituicao
   *
   ...
   * @throws ParameterException
   */
  public static function getInstanciaPorInstituicaoAno(Instituicao $oInstituicao, $iAno) {

    $iAno = (int)$iAno;
    $iCodigoInstituicao = $oInstituicao->getCodigo();
    if (empty($iAno) || empty($iCodigoInstituicao)) {
      throw new ParameterException("Parâmetro informados para o método [getInstanciaPorAnoInstituicao] inválidos.");
    }

    $aWhere = array(
      "instituicao = {$iCodigoInstituicao}",
      "ano = {$iAno}"
    );
    $oDaoProgramacao      = new cl_orcamentoprogramacaofinanceira();
    $sSqlBuscaProgramacao = $oDaoProgramacao->sql_query_file(null, 'sequencial', null, implode(' and ', $aWhere));
    $rsBuscaProgramacao   = $oDaoProgramacao->sql_record($sSqlBuscaProgramacao);
    if (!$rsBuscaProgramacao || $oDaoProgramacao->erro_status == "0" || $oDaoProgramacao->numrows == 0) {
      throw new BusinessException("Não foi localizada Programação Financeira para o ano {$iAno} e instituição {$oInstituicao->getDescricao()}.");
    }
    return new self(db_utils::fieldsMemory($rsBuscaProgramacao, 0)->sequencial);
  }
}