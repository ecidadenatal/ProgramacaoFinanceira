...
try {
  ...
  switch($oParam->exec) {
    ...
    case 'alterarSituacao':
      ...
      if (in_array($iSituacao, array(ResolucaoInteradministrativa::SITUACAO_APROVADA, ResolucaoInteradministrativa::SITUACAO_FECHADA))) {

        $oInstituicao = InstituicaoRepository::getInstituicaoByCodigo(db_getsession('DB_instit'));

        $oProgramacaoFinanceira = ProgramacaoFinanceira::getInstanciaPorInstituicaoAno($oInstituicao, $oDataAtual->getAno());
        $oFechamentoMensal      = new FechamentoMensal($oProgramacaoFinanceira);
        if ($oFechamentoMensal->mesFechado($oDataAtual->getMes())) {

          $sMes = DBDate::getMesExtenso($oDataAtual->getMes());
          throw new BusinessException("Não é possível alterar a Situação da RI, pois a Programação Financeira já está fechada para o mês de {$sMes}.");
        }

        $iUltimoMesFechado = $oFechamentoMensal->getUltimoMesFechado();
        foreach ($oResolucaoInteradministrativa->getFichas() as $oFicha) {

          $iMesFicha = $oFicha->getMes();
          if (!empty($iMesFicha) && $iMesFicha <= $iUltimoMesFechado) {

            $sMes = DBDate::getMesExtenso($iMesFicha);
            throw new BusinessException("Não é possível alterar a Situação da RI, pois existem fichas desta RI vinculadas a meses em que a Programação Financeira já está fechada.");
          }
        }
      }
